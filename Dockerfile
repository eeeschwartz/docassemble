...
RUN DEBIAN_FRONTEND=noninteractive TERM=xterm cd /tmp && wget https://github.com/jgm/pandoc/releases/download/2.5/pandoc-2.5-1-amd64.deb && dpkg -i pandoc-2.5-1-amd64.deb && rm pandoc-2.5-1-amd64.deb && mkdir -p /etc/ssl/docassemble /usr/share/docassemble/local /usr/share/docassemble/certs /usr/share/docassemble/backup /usr/share/docassemble/config /usr/share/docassemble/webapp /usr/share/docassemble/files /var/www/.pip /var/www/.cache /usr/share/docassemble/log /tmp/docassemble /var/www/html/log && echo '{ "args": ["--no-sandbox"] }' > /var/www/puppeteer-config.json && chown -R www-data.www-data /var/www && chsh -s /bin/bash www-data && update-alternatives --install /usr/bin/node node /usr/bin/nodejs 10 && wget -qO- https://deb.nodesource.com/setup_6.x | bash - && apt-get -y install nodejs && npm install -g azure-storage-cmd && npm install -g mermaid.cli
RUN DEBIAN_FRONTEND=noninteractive TERM=xterm cd /usr/share/docassemble && git clone https://github.com/letsencrypt/letsencrypt && cd letsencrypt && ./letsencrypt-auto --help && echo "host   all   all  0.0.0.0/0   md5" >> /etc/postgresql/9.6/main/pg_hba.conf && echo "listen_addresses = '*'" >> /etc/postgresql/9.6/main/postgresql.conf
COPY . /tmp/docassemble/
RUN DEBIAN_FRONTEND=noninteractive TERM=xterm ln -s /var/mail/mail /var/mail/root && cp /tmp/docassemble/docassemble_webapp/docassemble.wsgi /usr/share/docassemble/webapp/ && cp /tmp/docassemble/Docker/*.sh /usr/share/docassemble/webapp/ && cp /tmp/docassemble/Docker/VERSION /usr/share/docassemble/webapp/ && cp /tmp/docassemble/Docker/pip.conf /usr/share/docassemble/local/ && cp /tmp/docassemble/Docker/config/* /usr/share/docassemble/config/ && cp /tmp/docassemble/Docker/cgi-bin/index.sh /usr/lib/cgi-bin/ && cp /tmp/docassemble/Docker/syslog-ng.conf /usr/share/docassemble/webapp/syslog-ng.conf && cp /tmp/docassemble/Docker/syslog-ng-docker.conf /usr/share/docassemble/webapp/syslog-ng-docker.conf && cp /tmp/docassemble/Docker/docassemble-syslog-ng.conf /usr/share/docassemble/webapp/docassemble-syslog-ng.conf && cp /tmp/docassemble/Docker/apache.logrotate /etc/logrotate.d/apache2 && cp /tmp/docassemble/Docker/docassemble.logrotate /etc/logrotate.d/docassemble && cp /tmp/docassemble/Docker/cron/docassemble-cron-monthly.sh /etc/cron.monthly/docassemble && cp /tmp/docassemble/Docker/cron/docassemble-cron-weekly.sh /etc/cron.weekly/docassemble && cp /tmp/docassemble/Docker/cron/docassemble-cron-daily.sh /etc/cron.daily/docassemble && cp /tmp/docassemble/Docker/cron/docassemble-cron-hourly.sh /etc/cron.hourly/docassemble && cp /tmp/docassemble/Docker/docassemble.conf /etc/apache2/conf-available/ && cp /tmp/docassemble/Docker/docassemble-behindlb.conf /etc/apache2/conf-available/ && cp /tmp/docassemble/Docker/docassemble-supervisor.conf /etc/supervisor/conf.d/docassemble.conf && cp /tmp/docassemble/Docker/ssl/* /usr/share/docassemble/certs/ && cp /tmp/docassemble/Docker/rabbitmq.config /etc/rabbitmq/ && cp /tmp/docassemble/Docker/config/exim4-router /etc/exim4/conf.d/router/101_docassemble && cp /tmp/docassemble/Docker/config/exim4-filter /etc/exim4/docassemble-filter && cp /tmp/docassemble/Docker/config/exim4-main /etc/exim4/conf.d/main/01_docassemble && cp /tmp/docassemble/Docker/config/exim4-acl /etc/exim4/conf.d/acl/29_docassemble && cp /tmp/docassemble/Docker/config/exim4-update /etc/exim4/update-exim4.conf.conf && update-exim4.conf && bash -c "chown www-data.www-data /usr/share/docassemble/config && chown www-data.www-data /usr/share/docassemble/config/config.yml.dist /usr/share/docassemble/webapp/docassemble.wsgi && chown -R www-data.www-data /tmp/docassemble /usr/share/docassemble/local /usr/share/docassemble/log /usr/share/docassemble/files && chmod ogu+r /usr/share/docassemble/config/config.yml.dist && chmod 755 /etc/ssl/docassemble && cd /tmp && wget https://bootstrap.pypa.io/get-pip.py && python get-pip.py && rm -f get-pip.py && pip install --upgrade virtualenv"
